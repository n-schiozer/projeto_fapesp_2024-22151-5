...
class TRUSectorSector:
    ...
    data: TRUData
    market_share_domestic: pd.DataFrame
    production_sector_sector: pd.DataFrame
    intermediate_consumption_sector: pd.DataFrame
    final_demand_components_sector: pd.DataFrame
    exports_sector: pd.DataFrame
    gov_cons_sector: pd.DataFrame
    npo_consumption_sector: pd.DataFrame
    household_consumption_sector: pd.DataFrame
    gross_investment_sector: pd.DataFrame
    stocks_investment_sector: pd.DataFrame
    final_demand_sector: pd.DataFrame
    total_demand_with_margins_sector: pd.DataFrame
    trade_margin_sector: pd.DataFrame
    transport_margin_sector: pd.DataFrame
    taxes_sector: pd.DataFrame
    imports_sector: pd.DataFrame
    margins_and_taxes_sector: pd.DataFrame
    sector_demand_basic_prices: pd.DataFrame
    final_demand_basic_prices: pd.DataFrame
    value_added_components_sector: pd.DataFrame
    value_added_sector: pd.DataFrame
    wages_sector: pd.DataFrame
    profit_sector: pd.DataFrame
    technical_coefficients_sector: pd.DataFrame
    leontief_matrix: pd.DataFrame
    leontief_inverse: pd.DataFrame
    output_sector: pd.DataFrame
    accounting_gap: pd.DataFrame

    def to_excel_dataframe(self) -> pd.DataFrame:
        """Return the two side-by-side blocks used in the reference workbook."""

        sector_codes = [
            name.split(" - ", 1)[0] or str(index + 1)
        ...
        check = pd.DataFrame({
            "Check": self.accounting_gap.to_numpy(dtype=float).ravel()
        })
        return pd.concat(
...
def _clean_sector_name(value: object) -> str:
    """Transforma ``F\nConstrução`` em ``F - Construção``."""
...
def _sector_vector(values: np.ndarray, sector_names: list[str]) -> pd.DataFrame:
    """Vetor-coluna com os setores identificados nas linhas."""

    return pd.DataFrame(
        np.asarray(values, dtype=float).reshape(-1, 1),
        index=pd.Index(sector_names, name="setor"),
        columns=["valor"],
    )
...
def _sector_matrix(
    ...
    sector_names: list[str],
    *,
    row_name: str = "setor_fornecedor",
    column_name: str = "setor_consumidor",
) -> pd.DataFrame:
    """Matriz setor x setor com nomes explícitos nos dois eixos."""

    return pd.DataFrame(
        np.asarray(values, dtype=float),
        index=pd.Index(sector_names, name=row_name),
        columns=pd.Index(sector_names, name=column_name),
    )
...
def transform_tru_to_sector_sector(
    ...
    *,
    ...
    # Valor da produção por setor, já disponível na tabela de VA.
    output_sector = (
        value_added_components[[output_row], :]
        .T
        .copy()
    )
    ...
    result = TRUSectorSector(
        ...
        market_share_domestic=pd.DataFrame(
            market_share_domestic,
            index=pd.Index(data.product_names, name="produto"),
            columns=pd.Index(data.sector_names, name="setor_produtor"),
        ),
        production_sector_sector=_sector_matrix(
            production_sector_sector,
            data.sector_names,
            row_name="setor_produto",
            column_name="setor_atividade",
        ),
        intermediate_consumption_sector=_sector_matrix(
            intermediate_consumption_sector,
            data.sector_names,
        ),
        final_demand_components_sector=pd.DataFrame(
            final_demand_components_sector,
            index=pd.Index(data.sector_names, name="setor"),
            columns=[
                "exportacoes",
                "consumo_governo",
                "consumo_isflsf",
        ...
        exports_sector=_sector_vector(exports_sector, data.sector_names),
        gov_cons_sector=_sector_vector(gov_cons_sector, data.sector_names),
        npo_consumption_sector=_sector_vector(
            npo_consumption_sector, data.sector_names
        ),
        household_consumption_sector=_sector_vector(
            household_consumption_sector, data.sector_names
        ),
        gross_investment_sector=_sector_vector(
            gross_investment_sector, data.sector_names
        ),
        stocks_investment_sector=_sector_vector(
            stocks_investment_sector, data.sector_names
        ),
        final_demand_sector=_sector_vector(
            final_demand_sector, data.sector_names
        ),
        total_demand_with_margins_sector=_sector_vector(
            total_demand_with_margins_sector, data.sector_names
        ),
        trade_margin_sector=_sector_vector(
            trade_margin_sector, data.sector_names
        ),
        transport_margin_sector=_sector_vector(
            transport_margin_sector, data.sector_names
        ),
        taxes_sector=_sector_vector(taxes_sector, data.sector_names),
        imports_sector=_sector_vector(imports_sector, data.sector_names),
        margins_and_taxes_sector=_sector_vector(
            margins_and_taxes_sector, data.sector_names
        ),
        sector_demand_basic_prices=_sector_vector(
            sector_demand_basic_prices, data.sector_names
        ),
        final_demand_basic_prices=_sector_vector(
            final_demand_basic_prices, data.sector_names
        ),
        value_added_components_sector=pd.DataFrame(
            value_added_components.copy(),
            index=pd.Index(data.va_components_names, name="componente_va"),
            columns=pd.Index(data.sector_names, name="setor"),
        ),
        value_added_sector=_sector_vector(
            value_added_sector, data.sector_names
        ),
        wages_sector=_sector_vector(wages_sector, data.sector_names),
        profit_sector=_sector_vector(profit_sector, data.sector_names),
        technical_coefficients_sector=_sector_matrix(
            technical_coefficients_sector, data.sector_names
        ),
        leontief_matrix=_sector_matrix(
            leontief_matrix, data.sector_names
        ),
        leontief_inverse=_sector_matrix(
            leontief_inverse, data.sector_names
        ),
        output_sector=_sector_vector(output_sector, data.sector_names),
        accounting_gap=_sector_vector(accounting_gap, data.sector_names),
    )

    if validate:

        # ==========================================================
        # 1. COMPONENTES DA DEMANDA FINAL
        # ==========================================================
        # A soma dos componentes da demanda final deve reproduzir
        # a demanda final total por setor.
        component_gap = (
            final_demand_components_sector.sum(
                axis=1,
                keepdims=True,
            )
            - final_demand_sector
        ...
        # 2. USO TOTAL
        # ==========================================================
        # Demanda final + consumo intermediário por produto/setor
        # deve reproduzir o uso total a preços de comprador.
        total_use_gap = (
            final_demand_sector
            + intermediate_consumption_sector.sum(
                axis=1,
                keepdims=True,
            )
            - total_demand_with_margins_sector
        ...
        # ==========================================================
        # 3. PRODUÇÃO OBSERVADA DIRETAMENTE NA TABELA DE PRODUÇÃO
        # ==========================================================
        # A matriz original é produto x setor. Somando os produtos
        # em cada coluna obtemos o valor da produção de cada setor.
        output_from_production = (
            production.sum(
                axis=0,
                keepdims=True,
            ).T
        ...
        # ==========================================================
        # 4. PRODUÇÃO OBSERVADA NA TABELA DE VA
        # ==========================================================
        # output_sector foi extraído diretamente da linha
        # "Valor da produção" da tabela de VA.
        output_from_va = output_sector

        # ==========================================================
        # 5. FECHAMENTO DA MATRIZ DE PRODUÇÃO TRANSFORMADA
        # ==========================================================
        # production_sector_sector = D @ production.
        #
        # As somas por linha e por coluna devem reproduzir o
        # mesmo vetor de produção setorial.
        output_from_transformed_rows = (
            production_sector_sector.sum(
                axis=1,
                keepdims=True,
            )
        )

        output_from_transformed_columns = (
            production_sector_sector.sum(
                axis=0,
                keepdims=True,
            ).T
        ...
        # 6. LEONTIEF
        # ==========================================================
        # A matriz técnica desta função foi calibrada usando
        # total_demand_with_margins_sector como denominador.
        #
        # Portanto:
        #
        #     (I - A) g = f
        ...
        #     g = L @ f
        #
        # onde g é o uso total a preços de comprador.
        leontief_output = (
            leontief_inverse
            @ final_demand_sector
        )
        ...
        checks = {
            ...
            # DF = soma dos componentes da DF.
            "componentes da demanda final":
                component_gap,

            # Uso total = DF + CI.
            "demanda total por setor":
                total_use_gap,

            # Produção Pb = CI Pc + VA Pb.
            "identidade producao-CI-VA":
                accounting_gap,

            # Valor da produção obtido diretamente da tabela
            # produto x setor deve coincidir com a linha de produção
            # da tabela de VA.
            "producao: tabela de producao x tabela VA":
                output_from_production
                - output_from_va,

            # A oferta doméstica obtida após retirar importações,
            # impostos e margens do uso total deve reproduzir a
            # produção observada na tabela de VA.
            "producao: oferta-utilizacao x tabela VA":
                sector_demand_basic_prices
                - output_from_va,

            # Mesma comparação, agora contra a soma direta da
            # matriz produto x setor.
            "producao: oferta-utilizacao x tabela de producao":
                sector_demand_basic_prices
                - output_from_production,

            # A matriz setor x setor obtida pela transformação deve
            # preservar o vetor de produção.
            "producao transformada: soma das linhas":
                output_from_transformed_rows
                - output_from_production,

            "producao transformada: soma das colunas":
                output_from_transformed_columns
                - output_from_production,

            # Dada a definição de A nesta função, L @ DF deve
            # reproduzir o uso total a preços de comprador.
            "fechamento de Leontief":
                leontief_output
                - total_demand_with_margins_sector,
        }
        ...
        if failures:
            details = ", ".join(
                f"{name}={gap:.6g}"
                for name, gap in failures.items()
            )

            raise ValueError(
                "A TRU setor x setor nao fechou: "
                f"{details}."
            )
