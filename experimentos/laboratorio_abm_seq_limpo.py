...
CONFIG_ABM = {
    ...
    "choques_climaticos": {
        ...
        "setores": {
            "A - Agricultura, pecuária, produção florestal, pesca e aquicultura": {
                "periodo_choque": 5,
                "multiplicador_produtividade": 0.8,
                "choque_permanente": True,
            },

            "D - Eletricidade e gás": {
                "periodo_choque": 5,
                "multiplicador_produtividade": 0.9,
                "choque_permanente": True,
            },
        },
    },
...
preco_capital_base = 1

# 1. Calcula r_obs do ano-base
...
for setor in setores_nf:
    ...
    r_obs_base_setorial = (
        eob_base_setorial
        / (preco_capital_base * capital_base_setorial)
        - depreciacao_capital_nf
    )

    taxa_retorno_parametro_setorial[setor] = (
        r_obs_base_setorial
        - float(cfg["taxa_juros_real"])
    )
...
for firma in firmas.values():
    ...
    firma.taxa_retorno_parametro = (
        taxa_retorno_parametro_setorial[firma.setor]
    )

    # Agora que r_param existe, recalcula r_ajustado.
    firma.calcular_taxa_retorno_observada(
        preco_capital=preco_capital_base,
        depreciacao=depreciacao_capital_nf,
        taxa_juros_real=float(cfg["taxa_juros_real"]),
...
for t in range(1, periodos + 1):
    ...
    for firma in firmas.values():
        ...
        # ==========================================================
        # TESTE DA PRODUÇÃO APÓS O CHOQUE
        # ==========================================================
   ...
   # ==========================================================
    ...
    for firma in firmas.values():
        ...
        if firma.id in ["D_001"] and t <= 10:

            print(
                f"t={t} | {firma.id}"
                f" | r_obs={firma.taxa_retorno_observada:.6f}"
                f" | r_param={firma.taxa_retorno_parametro:.6f}"
                f" | r_BC={taxa_juros_real:.6f}"
                f" | r_adj={firma.taxa_retorno_ajustada:.6f}"
            )
